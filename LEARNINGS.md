...
- ~~**Video coaching phase.**~~ **Closed 2026-05-04.** Shipped Phase 1 (video capture, [`ba35c3a`](https://github.com/IanHSDavis/elevator-pitch-evaluator/commit/ba35c3a)) and Phase 2 (Claude-multimodal visual coaching across Presence / Eye Contact / Delivery Confidence, [`f86981a`](https://github.com/IanHSDavis/elevator-pitch-evaluator/commit/f86981a)) on the same day. All cost mitigations from the plan landed: 4 keyframes at 640×480, visual rubric in the cached system prompt, image blocks before transcript text. Coached, not scored.
- **Visual coaching calibration.** The audio rubric got an n=20 borderline study with stdev 0; the three visual dims shipped without that discipline. They're qualitative-not-scored, which softens the variance bar — but coherence of the coaching prose across runs (does the same four-frame sample yield consistent presence/eye-contact diagnosis?) is the analogous check, and it's the honest interview answer to "how did you validate the visual layer?" Open.
- **v5 coaching candidate — full rewrites for rubric-clean pitches.** From the v4 strong-pitch critique: *"Three 'try' suggestions, zero rewrites of the actual pitch."* When `overall_impression` diagnoses a rubric-clean-with-tactical-issues pitch, end with one full rewritten version the rep can read aloud Monday. Same shape as the v3 praise-inflation guard — gates on rubric satisfaction so weak/mid reps still do the work themselves. Open.
- **Wider pitch corpus.** Three demo pitches are not enough to claim the coaching generalizes. A bigger corpus (10–20 pitches across role, product category, and rubric-satisfaction band) would let me re-test the zero-variance question and surface the next dominant pattern beyond the v4 plateau.
